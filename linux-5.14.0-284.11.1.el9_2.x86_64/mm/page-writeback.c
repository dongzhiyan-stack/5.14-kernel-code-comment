			     ...
			     pgoff_t start, pgoff_t end)
{
	...
	xas_for_each_marked(&xas, page, end, PAGECACHE_TAG_DIRTY) {
        ...
        /*下边的代码没啥实质意义*/
        //tagged增加到XA_CHECK_SCHED，执行下边的"xas_pause(&xas)"等代码
		if (++tagged % XA_CHECK_SCHED)
			continue;
        //下边要xas_unlock_irq释放锁了，这里重置xas->xa_node=XAS_RESTART，更新xas->xa_index，确定下次查找的page的索引
		xas_pause(&xas);
		xas_unlock_irq(&xas);
        //调度
		cond_resched();
 ...
 */
//wb_writeback->__writeback_inodes_wb->writeback_sb_inodes->__writeback_single_inode->do_writepages->xfs_vm_writepages->iomap_writepages->write_cache_pages 
//刷脏页就是这里 iomap_writepages->write_cache_pages
...
continue_unlock:
			...
			error = (*writepage)(page, wbc, data);//  ext4_writepage
			if (unlikely(error)) {
				/*
			     ...
			     int warn)
{
	...
	if (folio->mapping) {	/* Race with truncate? */
        ...
        //设置page在xarray tree的dirty mark标记
...
bool __folio_start_writeback(struct folio *folio, bool keep_write)
{
	...
	if (mapping && mapping_use_writeback_tags(mapping)) {
        ...
        //如果page之前没有writeback标记则if成立
		if (!ret) {
			bool on_wblist;
            
			on_wblist = mapping_tagged(mapping,
						   PAGECACHE_TAG_WRITEBACK);
            //在xarray tree设置page的 writeback mark
        ...
        //如果page不是脏页，在xarray tree清理page的 dirty mark
		if (!folio_test_dirty(folio))
			xas_clear_mark(&xas, PAGECACHE_TAG_DIRTY);
     
		if (!keep_write)//在xarray tree清理page的 towrite mark
