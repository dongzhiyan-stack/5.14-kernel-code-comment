...
static void read_pages(struct readahead_control *rac)
{
	...
	if (aops->readahead) {
        //xfs_vm_readahead->iomap_readahead->iomap_readpage_iter->readahead_page->__readahead_folio，这里调用xa_load()从xarray tree查找page
		aops->readahead(rac);//xfs_vm_readahead
		/*
		...
		unsigned long nr_to_read, unsigned long lookahead_size)
{
	 ...
	 */
	read_pages(ractl);//这里调用 iomap_readahead
		...
		unsigned long nr_to_read, unsigned long lookahead_size)
{
	...
	page_cache_ra_unbounded(ractl, nr_to_read, lookahead_size);//这里调用 read_pages
}
				   ...
				   pgoff_t index, unsigned long max)
{
	...
	rcu_read_lock();
    //从index -1 索引一直在xarray tree向前找，找到第一个空洞page，返回这个空洞page的索引给head
	head = page_cache_prev_miss(mapping, index - 1, max);
	rcu_read_unlock();
    //返回值是空洞page索引到起始索引index 之间的page数吧。如果第一个空洞page就是index这个page，返回值是0
	return index - 1 - head;
}
...
{
	pgoff_t size;
    //里边调用 page_cache_prev_miss()，从index索引一直在xarray tree向前找，找到第一个空洞page。然后
    //返回值是空洞page索引到起始索引index 之间的page数吧。如果第一个空洞page就是index这个page，返回值是0，也就是index这个page也不存在
	 ...
	 */
	//如果index这个page就不存在，则size就是0，if成立返回0
	if (size <= req_size)
		return 0;
		...
		struct folio *folio, unsigned long req_size)
{
	...
	if (folio) {
		...
		rcu_read_lock();
        //从index + 1索引一直在xarray tree向后找，找到第一个空洞page，即这个槽位没有page,返回这个索引给start
	 ...
	 */
	//这里调用page_cache_prev_miss()根据index前的第一个空洞page索引更新ra->size，ra->start赋值index
	//如果index这个page也不存在，则返回0。如果index这个page存在返回1,if成立
	 ...
	 */
	do_page_cache_ra(ractl, req_size, 0);//这里调用 page_cache_ra_unbounded
	return;
		...
		struct folio *folio, unsigned long req_count)
{
	...
	ondemand_readahead(ractl, folio, req_count);//这里调用 do_page_cache_ra
}
