				...
				loff_t lstart, loff_t lend)
{
	...
	struct folio_batch fbatch;//pagevec
	...
	while (index < end && find_lock_entries(mapping, index, end - 1,
        ...
        //针对writeback、dirty、mmap、folio_has_private page的处理
	...
	index = start;
    //这里再最终截断文件的所有文件页，但貌似不能保证把所有的文件页都截断
	while (index < end) {
		cond_resched();
		...
		pgoff_t start, pgoff_t end, unsigned long *nr_pagevec)
{
	...
	folio_batch_init(&fbatch);
    //先find_lock_entries从radix tree遍历得到15个page，并加锁
	while (find_lock_entries(mapping, index, end, &fbatch, indices)) {
		...
		for (i = 0; i < folio_batch_count(&fbatch); i++) {
            ...
            //对mmap、write、dirty page的处理，并且把page从radix tree剔除
			 ...
			 */
			//到这里说明page内存回收失败了，按照page的属性再移动回lru链表
			if (!ret) {
