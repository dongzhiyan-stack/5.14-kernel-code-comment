 ...
 */
//xas_set_mark()中设置文件xarray tree的mark标记，xas_clear_mark()中清理文件xarray tree的mark标记
...
struct xa_state {
	struct xarray *xa;
	unsigned long xa_index;//要查找的page的起始索引
	unsigned char xa_shift;//一般是0
	unsigned char xa_sibs;//一般是0
	unsigned char xa_offset;//要查找的page在父节点的槽位偏移
	unsigned char xa_pad;		/* Helps gcc generate better code */
	struct xa_node *xa_node;//父节点
 ...
 */
//entry被标记retry是绝对存在的，当一个父节点被delete，它的子成员就被标记retry。因此一旦遍历
//xarray tree时发现是个retry entry，则要重新遍历xarray tree
...
static inline void xas_set_order(struct xa_state *xas, unsigned long index,
					unsigned int order)
{
#ifdef CONFIG_XARRAY_MULTI//yes
    //正常情况跟XA_STATE(xas, &mapping->i_pages, folio->index)初始化后的xax效果是一样的
	xas->xa_index = order < BITS_PER_LONG ? (index >> order) << order : 0;//xas->xa_index = index
	xas->xa_shift = order - (order % XA_CHUNK_SHIFT);//xas->xa_shift = 0
	xas->xa_sibs = (1 << (order % XA_CHUNK_SHIFT)) - 1;//xas->xa_sibs = 0;
	xas->xa_node = XAS_RESTART;
#else
	BUG_ON(order > 0);
	xas_set(xas, index);
 ...
 */
//以xas->xa_index为起始索引，一直向后查找，直到找到非NULL的page才会退出循环。或者，要查找的page索引大于xas->xa_offset大于max也退出循环
static inline void *xas_next_entry(struct xa_state *xas, unsigned long max)
{
	...
	do {
		...
		if (unlikely(xas->xa_offset == XA_CHUNK_MASK))
			return xas_find(xas, max);
        //查找xas->xa_offset + 1 加1后的槽位的page
		entry = xa_entry(xas->xa, node, xas->xa_offset + 1);
		if (unlikely(xa_is_internal(entry)))
			return xas_find(xas, max);
        //令xas->xa_offset 和 xas->xa_index 加1
		xas->xa_offset++;
		xas->xa_index++;
	} while (!entry);
...
{
	struct xa_node *node = xas->xa_node;
    //如果xas->xa_node 是XAS_RESTART 或者 xas->xa_offset == 63 ，则重新从xrray tree查找page
	if (unlikely(xas_not_node(node) || node->shift ||
				xas->xa_offset == XA_CHUNK_MASK))
		return __xas_next(xas);

    //否则，令xas->xa_index加1，这个指定了从xarray tree查找page的索引
	xas->xa_index++;
    //否则，令xas->xa_offset加1，下次从父节点xa_node->slots[xas->xa_offset]查找page
	xas->xa_offset++;
	return xa_entry(xas->xa, node, xas->xa_offset);
}
