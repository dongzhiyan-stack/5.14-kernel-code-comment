...
{
	unsigned int shift = xas->xa_node->shift;//0
	xas->xa_index &= ~XA_CHUNK_MASK << shift;//就是对xas->xa_index低6位清0吧
	xas->xa_index += offset << shift;//令xas->xa_index低6位加上offset
}
//这个函数的本质作用就是xas->xa_offset加1、xas->xa_index加1
static void xas_next_offset(struct xa_state *xas)
{
	xas->xa_offset++;
    //这里本质就是xas->xa_index加1，不要被代码迷惑了
	xas_move_index(xas, xas->xa_offset);
}
//在xas_start()里调用，要扩建xarray tree
...
static void *xas_start(struct xa_state *xas)
{
    ...
    //根节点不是node节点，即根节点保存是索引是0的page指针
	if (!xa_is_node(entry)) {
        //此时查找的文件页page大于0，那就set_bounds()设置xas->xa_node = XAS_BOUNDS，目的是扩建xarray tree。
		if (xas->xa_index)
			return set_bounds(xas);
	} else {
	    //如果要查找的文件页page索引大于xarray tree保存的最大的page的索引，if成立。那就set_bounds()设置xas->xa_node = XAS_BOUNDS，目的是扩建xarray tree。
 ...
 */
//对保存page指针的父节点node->marks[mark]数组变量的offset这个bit位设置mark标记，然后逐级对每一个上层父节点
//node->marks[mark]数组变量的offset这个bit位设置mark标记。直到达到根节点node是NULL，或者某一级的父节点已经设置过了则退出
void xas_set_mark(const struct xa_state *xas, xa_mark_t mark)
{
    //node此时已经是父节点
	struct xa_node *node = xas->xa_node;
    //xas->xa_offset这个父节点槽位位置保存了page指针，现在要在父节点node->marks[mark]数组变量的offset这个bit位设置mark标记
	unsigned int offset = xas->xa_offset;
 ...
 */
//对保存page指针的父节点node->marks[mark]数组变量的offset这个bit位 清理 mark标记，然后逐级对每一个上层父节点
//node->marks[mark]数组变量的offset这个bit位 清理 mark标记。直到达到根节点node是NULL，或者某一级的父节点已经清理过了则结束
void xas_clear_mark(const struct xa_state *xas, xa_mark_t mark)
{
    //node此时已经是父节点
	struct xa_node *node = xas->xa_node;
    //xas->xa_offset这个父节点槽位位置保存了page指针，现在要在父节点node->marks[mark]数组变量的offset这个bit位清理mark标记
	unsigned int offset = xas->xa_offset;
    ...
    /*只有xarray tree的所有mark都被消除，才会执行到这里消除xarray tree整个mark标记*/
    //如果该文件xarray tree还没有被标记mark则标记
	if (xa_marked(xas->xa, mark))
...
void xas_pause(struct xa_state *xas)
{
    ...
    //重置xas->xa_node
	xas->xa_node = XAS_RESTART;
    //下边是更新xas->xa_index，确定下次查找的page的索引
	if (node) {
 ...
 */
//按照xas->xa_offset起始索引从xarray tree查找page，找不到则xas->xa_offset加1继续查找，直到查找第一个有效的page。或者
//xas->xa_offset大于max还是没有找到有效page，则返回NULL。find_get_entry()函数里有对xas_find()更详细的讲解
void *xas_find(struct xa_state *xas, unsigned long max)
{
	...
	if (!xas->xa_node) {
		xas->xa_index = 1;
		return set_bounds(xas);
    //第一次执行xas_find，xas->xa_node是初始值XAS_RESTART 而这个if成立。然后第2次执行xas_find，这个if不成立，
    //而会执行下边的xas_next_offset(xas)令xas->xa_offset加1、xas->xa_index加1
	} else if (xas->xa_node == XAS_RESTART) {
	    //根据xas->xa_index查找page，查找成功直接return
		entry = xas_load(xas);
    ...
    //这个函数的本质作用就是xas->xa_offset加1、xas->xa_index加1，表示要查找下一个page
	xas_next_offset(xas);

    //注意，这里会陷入死循环，一直查找page，找不到则xas->xa_offset加1，直到xas->xa_index大于max
	while (xas->xa_node && (xas->xa_index <= max)) {
        //如果父节点的最后一个page已经遍历过了，那就令
		if (unlikely(xas->xa_offset == XA_CHUNK_SIZE)) {
            //找到当前父节点在父父节点槽位的下一个父节点的槽位偏移
			xas->xa_offset = xas->xa_node->offset + 1;// xas->xa_node->offset是父节点在父父节点的槽位偏移
			//找到当前父节点在父父节点槽位的下一个父节点，赋值给xas->xa_node，后续循环就是从这个新的父节点开始查找page
			xas->xa_node = xa_parent(xas->xa, xas->xa_node);
			continue;
		}
        //按照索引xas->xa_offset从xarray tree的父节点xas->xa_node的槽位查找page
		entry = xa_entry(xas->xa, xas->xa_node, xas->xa_offset);
        //如果entry是node节点，那把这个父节点更新给xas->xa_node，xas->xa_offset = 0清0。continue后下次循环
        //执行到"entry = xa_entry(xas->xa, xas->xa_node, xas->xa_offset)"，就是从这个新的父节点的槽位0开始开始查找page
		if (xa_is_node(entry)) {
			xas->xa_node = xa_to_node(entry);
			xas->xa_offset = 0;
			continue;
		...
		if (entry && !xa_is_sibling(entry))
			return entry;
        //这个函数的本质作用就是xas->xa_offset加1、xas->xa_index加1。为什么也要令xas->xa_index加1? 这个只是要查找的page的起始
        //索引，这里每找到一个page就令xas->xa_index1，目的是:如果当前函数某处调用xas_find()找到一个有效的page后，但是
        //if (xas_retry(xas,folio))成立，里边有xas->xa_node = XAS_RESTART。然后下次再执行xas_find()，
        //else if (xas->xa_node == XAS_RESTART)成立，于是执行entry = xas_load(xas)按照xas->xa_index去xarray tree重新查找page，
        //此时肯定不能按照最初的xas->xa_index去查找了，应该按照刚才"xas_find()找到一个有效的page后"时的page索引，去xarray tree
        //查找。而这里的xas_next_offset(xas)里的xas->xa_index加1，就能做到这一点。简单说，xas->xa_index加1的目的是:
        //当查找的page异常，需要重新执行xas_find()去查找page时，保证从刚才的page索引开始查找，而不是从最原始的page索引查找。
		xas_next_offset(xas);
	}
 ...
 * Return: The entry, if found, otherwise %NULL.
 */
//按照xas->xa_offset起始索引从xarray tree查找有mark标记的page，找不到则xas->xa_offset加1继续查找，直到查找第一个有效的page。或者
//xas->xa_offset大于max还是没有找到有效page，则返回NULL
...
int xa_get_order(struct xarray *xa, unsigned long index)
{
	...
	for (;;) {
		...
		if (slot >= XA_CHUNK_SIZE)
			break;
        //一般都不是sibling，这里break了
		if (!xa_is_sibling(xas.xa_node->slots[slot]))
			break;
		order++;
	}
    //xas.xa_node->shift是0，最终order是0，这个xa_get_order函数导致什么意义?????????????
